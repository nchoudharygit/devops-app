"""
DevOps Sample App - docker cpmpose
-----------------
...
def index():
    logger.info("GET /")
    return jsonify({
        "app": "devops-sample-app-docker-compose-docker-compose",
...
def info():
    ...
    return jsonify({
        "app": "devops-sample-app-docker-compose",
        "version": os.getenv("APP_VERSION", "1.0.0"),
        "environment": os.getenv("APP_ENV", "development"),
...
if __name__ == "__main__":
    port = int(os.getenv("PORT", 5000))
    logger.info(f"Starting devops-sample-app-docker-compose on port {port}")
